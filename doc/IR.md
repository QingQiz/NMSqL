...
|14|Put|Y|N|N|将栈顶元素作为值 将下一个元素作为键 写入游标P1 出栈两个元素,此时游标 P1 应指向新插入的数据位置|
...
|17|NotFound|Y|Y|N|出栈一个元素作为key 若在游标P1中不存在 则跳转到P2 游标指向此key或大于它的最小的元素|
|18|Delete|Y|N|N|将P1游标此时所指元素删除 Next类操作打标记|
